	...
	internal class Lsp
	{
		...
		public static int Lpc2lsp(float[] a, int lpcrdr, float[] freq, int nb, float delta)
		{
			...
			for (int j = 0; j < lpcrdr; j++)
			{
				...
				float num10 = Lsp.Cheb_poly_eva(array3, num9, lpcrdr);
				int num11 = 1;
				while (num11 == 1 && (double)num8 >= -1.0)
				{
					...
					num8 = num9 - num12;
					float num13 = Lsp.Cheb_poly_eva(array3, num8, lpcrdr);
					float num14 = num13;
					float num15 = num8;
					if ((double)(num13 * num10) < 0.0)
					{
						...
						{
							num = (num9 + num8) / 2f;
							float num16 = Lsp.Cheb_poly_eva(array3, num, lpcrdr);
		...
		// Token: 0x06000017 RID: 23 RVA: 0x00002A2C File Offset: 0x00000C2C
		...
		public static void Enforce_margin(float[] lsp, int len, float margin)
		{
			...
			if (lsp[len - 1] > 3.1415927f - margin)
			{
				lsp[len - 1] = 3.1415927f - margin;
			}
