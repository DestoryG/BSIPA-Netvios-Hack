...
namespace NSpeex
{
	...
	{
		// Token: 0x0600002C RID: 44 RVA: 0x000031CB File Offset: 0x000013CB
		...
		public sealed override int Quant(float[] target, float[] sw, int sws, float[] ak, float[] awk1, float[] awk2, float[] exc, int es, int start, int end, float pitch_coef, int p, int nsf, Bits bits, float[] exc2, int e2s, float[] r, int complexity)
		{
			...
			Ltp.Open_loop_nbest_pitch(sw, sws, start, end, nsf, array2, array3, num5);
			for (int i = 0; i < num5; i++)
			{
				...
				float num6 = this.Pitch_gain_search_3tap(target, ak, awk1, awk2, exc, es, num, p, nsf, bits, exc2, e2s, r, array);
				if (num6 < num4 || num4 < 0f)
				{
					for (int j = 0; j < nsf; j++)
					{
						array4[j] = exc[es + j];
			...
			bits.Pack(num3 - start, this.pitch_bits);
			bits.Pack(num2, this.gain_bits);
			for (int i = 0; i < nsf; i++)
			{
				exc[es + i] = array4[i];
			}
		...
		// Token: 0x06000030 RID: 48 RVA: 0x00003620 File Offset: 0x00001820
