...
namespace NSpeex
{
	...
	{
		// Token: 0x06000124 RID: 292 RVA: 0x00014E30 File Offset: 0x00013030
		public NbDecoder()
		{
		...
		// Token: 0x06000125 RID: 293 RVA: 0x00014E68 File Offset: 0x00013068
		...
		// Token: 0x06000126 RID: 294 RVA: 0x00014EC8 File Offset: 0x000130C8
		public virtual int Decode(Bits bits, float[] xout)
		{
			...
			for (int j = 0; j < this.nbSubframes; j++)
			{
				...
				if (this.enhanced)
				{
					...
					float num20 = (1f - (1f - num19 * lpcEnhK) / (1f - num19 * lpcEnhK2)) / num19;
					...
					Filters.Bw_lpc(num20, this.interp_qlpc, this.awk3, this.lpcSize);
				}
				num18 = 1f;
				this.pi_gain[j] = 0f;
				for (int i = 0; i <= this.lpcSize; i++)
				...
				if (this.submodes[this.submodeID].LbrPitch != -1)
				{
					...
					if (lbrPitch != 0)
					{
						num21 = num - lbrPitch + 1;
						if (num21 < this.min_pitch)
						{
							num21 = this.min_pitch;
						}
						int num22 = num + lbrPitch;
						if (num22 > this.max_pitch)
						{
							num22 = this.max_pitch;
						}
					}
					else
					{
				...
				{
					num21 = this.min_pitch;
					int num22 = this.max_pitch;
				}
				int num23 = this.submodes[this.submodeID].Ltp.Unquant(this.excBuf, num17, num21, num2, this.subframeSize, array, bits, this.count_lost, num15, this.last_pitch_gain);
				if (this.count_lost != 0 && num13 < this.last_ol_gain)
				{
					float num24 = num13 / (this.last_ol_gain + 1f);
					for (int i = 0; i < this.subframeSize; i++)
					{
						this.excBuf[this.excIdx + i] *= num24;
					}
				}
				...
				int num25 = j * this.subframeSize;
				for (int i = num25; i < num25 + this.subframeSize; i++)
				{
					this.innov[i] = 0f;
				}
				float num27;
				...
				{
					int num26 = bits.Unpack(3);
					num27 = (float)((double)num13 * Math.Exp((double)NbCodec.exc_gain_quant_scal3[num26]));
				}
				else if (this.submodes[this.submodeID].HaveSubframeGain == 1)
				{
					int num26 = bits.Unpack(1);
					num27 = (float)((double)num13 * Math.Exp((double)NbCodec.exc_gain_quant_scal1[num26]));
				}
				else
				{
					num27 = num13;
				...
				{
					this.submodes[this.submodeID].Innovation.Unquantify(this.innov, num25, this.subframeSize, bits);
				}
				for (int i = num25; i < num25 + this.subframeSize; i++)
				{
					this.innov[i] *= num27;
				...
				if (this.submodeID == 1)
				{
					...
					for (int i = 0; i < this.subframeSize; i++)
					{
						float num29 = this.excBuf[num17 + i];
						this.excBuf[num17 + i] = 0.8f * num28 * this.excBuf[num17 + i] * num13 + 0.6f * num28 * this.voc_m1 * num13 + 0.5f * num28 * this.innov[num25 + i] - 0.5f * num28 * this.voc_m2 + (1f - num28) * this.innov[num25 + i];
						this.voc_m1 = num29;
						this.voc_m2 = this.innov[num25 + i];
				...
				else
				{
					for (int i = 0; i < this.subframeSize; i++)
					{
						this.excBuf[num17 + i] += this.innov[num25 + i];
					}
				}
				if (this.submodes[this.submodeID].DoubleCodebook != 0)
				{
					...
					for (int i = 0; i < this.subframeSize; i++)
					{
						this.innov2[i] *= num27 * 0.45454544f;
					}
					for (int i = 0; i < this.subframeSize; i++)
					{
				...
				{
					this.filters.Comb_filter(this.excBuf, num17, this.frmBuf, num16, this.subframeSize, num23, array, this.submodes[this.submodeID].CombGain);
				}
				if (this.enhanced)
				{
		...
		// Token: 0x06000127 RID: 295 RVA: 0x00015C94 File Offset: 0x00013E94
		public int DecodeLost(float[] xout)
		{
			...
			for (int i = 0; i < this.nbSubframes; i++)
			{
				...
				if (this.enhanced)
				{
					...
					float num10 = (1f - (1f - num7 * num8) / (1f - num7 * num9)) / num7;
					Filters.Bw_lpc(num8, this.interp_qlpc, this.awk1, this.lpcSize);
					Filters.Bw_lpc(num9, this.interp_qlpc, this.awk2, this.lpcSize);
					Filters.Bw_lpc(num10, this.interp_qlpc, this.awk3, this.lpcSize);
				}
				float num11 = 0f;
				for (int j = 0; j < this.frameSize; j++)
				{
					num11 += this.innov[j] * this.innov[j];
				}
				num11 = (float)Math.Sqrt((double)(num11 / (float)this.frameSize));
				for (int j = 0; j < this.subframeSize; j++)
				{
					this.excBuf[num6 + j] = num3 * this.excBuf[num6 + j - this.last_pitch] + num * (float)Math.Sqrt((double)(1f - num3)) * 3f * num11 * ((float)this.random.NextDouble() - 0.5f);
				}
				for (int j = 0; j < this.subframeSize; j++)
				{
		...
		// Token: 0x06000128 RID: 296 RVA: 0x000160D8 File Offset: 0x000142D8
		...
		// (get) Token: 0x06000129 RID: 297 RVA: 0x000160E7 File Offset: 0x000142E7
		public bool Dtx
		{
			get
			{
		...
		// (get) Token: 0x0600012A RID: 298 RVA: 0x000160F5 File Offset: 0x000142F5
		// (set) Token: 0x0600012B RID: 299 RVA: 0x000160FD File Offset: 0x000142FD
