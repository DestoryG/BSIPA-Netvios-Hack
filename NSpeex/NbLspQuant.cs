...
namespace NSpeex
{
	...
	internal class NbLspQuant : LspQuant
	{
		...
		public sealed override void Quant(float[] lsp, float[] qlsp, int order, Bits bits)
		{
			...
			int num3 = LspQuant.Lsp_quant(qlsp, 0, Codebook_Constants.cdbk_nb, 64, order);
			bits.Pack(num3, 6);
			for (int i = 0; i < order; i++)
			{
				qlsp[i] *= 2f;
			}
			num3 = LspQuant.Lsp_weight_quant(qlsp, 0, array, 0, Codebook_Constants.cdbk_nb_low1, 64, 5);
			bits.Pack(num3, 6);
			for (int i = 0; i < 5; i++)
			{
				qlsp[i] *= 2f;
			}
			num3 = LspQuant.Lsp_weight_quant(qlsp, 0, array, 0, Codebook_Constants.cdbk_nb_low2, 64, 5);
			bits.Pack(num3, 6);
			num3 = LspQuant.Lsp_weight_quant(qlsp, 5, array, 5, Codebook_Constants.cdbk_nb_high1, 64, 5);
			bits.Pack(num3, 6);
			for (int i = 5; i < 10; i++)
			{
				qlsp[i] *= 2f;
			}
			num3 = LspQuant.Lsp_weight_quant(qlsp, 5, array, 5, Codebook_Constants.cdbk_nb_high2, 64, 5);
			bits.Pack(num3, 6);
			for (int i = 0; i < order; i++)
			{
		...
		// Token: 0x060000E1 RID: 225 RVA: 0x00012564 File Offset: 0x00010764
