...
namespace HarmonyLib
{
	...
	{
		// Token: 0x060000CC RID: 204 RVA: 0x0000645C File Offset: 0x0000465C
		...
		// Token: 0x060000CD RID: 205 RVA: 0x00006580 File Offset: 0x00004780
		internal MethodInfo CreateReplacement(out Dictionary<int, CodeInstruction> finalInstructions)
		{
			LocalBuilder[] array = this.DeclareLocalVariables(this.source ?? this.original);
			Dictionary<string, LocalBuilder> privateVars = new Dictionary<string, LocalBuilder>();
			LocalBuilder localBuilder = null;
			if (this.idx > 0)
			{
				localBuilder = this.DeclareLocalVariable(this.returnType);
			...
			{
				localBuilder2 = this.DeclareLocalVariable(typeof(bool));
			...
			this.prefixes.Union(this.postfixes).Union(this.finalizers).ToList<MethodInfo>()
				.ForEach(delegate(MethodInfo fix)
				{
					if (fix.DeclaringType != null && !privateVars.ContainsKey(fix.DeclaringType.FullName))
					{
						...
						{
							LocalBuilder localBuilder5 = this.DeclareLocalVariable(patchParam.ParameterType);
			...
			{
				localBuilder3 = this.DeclareLocalVariable(typeof(bool));
				privateVars["__exception"] = this.DeclareLocalVariable(typeof(Exception));
				Label? label2;
			...
			{
				LocalBuilder localBuilder4 = this.DeclareLocalVariable(this.returnType);
		...
		// Token: 0x060000CE RID: 206 RVA: 0x00006B3C File Offset: 0x00004D3C
		...
		private LocalBuilder[] DeclareLocalVariables(MethodBase member)
		{
			...
			return list.Select((LocalVariableInfo lvi) => this.il.DeclareLocal(lvi.LocalType, lvi.IsPinned)).ToArray<LocalBuilder>();
		}

		// Token: 0x060000D0 RID: 208 RVA: 0x00006D60 File Offset: 0x00004F60
		...
		// Token: 0x060000D2 RID: 210 RVA: 0x00006FDC File Offset: 0x000051DC
		...
		// Token: 0x060000D3 RID: 211 RVA: 0x0000707C File Offset: 0x0000527C
		private void EmitCallParameter(MethodInfo patch, Dictionary<string, LocalBuilder> variables, bool allowFirsParamPassthrough)
		{
		...
		// Token: 0x060000D4 RID: 212 RVA: 0x000076B8 File Offset: 0x000058B8
		...
		private void AddPrefixes(Dictionary<string, LocalBuilder> variables, LocalBuilder runOriginalVariable)
		{
			...
			this.prefixes.Do(delegate(MethodInfo fix)
			{
				...
				this.EmitCallParameter(fix, variables, false);
		...
		// Token: 0x060000D6 RID: 214 RVA: 0x00007760 File Offset: 0x00005960
		private bool AddPostfixes(Dictionary<string, LocalBuilder> variables, bool passthroughPatches)
		{
			...
			this.postfixes.Where((MethodInfo fix) => passthroughPatches == (fix.ReturnType != typeof(void))).Do(delegate(MethodInfo fix)
			{
				...
				this.EmitCallParameter(fix, variables, true);
				this.emitter.Emit(OpCodes.Call, fix);
		...
		// Token: 0x060000D7 RID: 215 RVA: 0x000077C0 File Offset: 0x000059C0
