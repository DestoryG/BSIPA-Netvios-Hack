	...
	internal class RuntimeType : global::System.Reflection.TypeInfo, ISerializable, ICloneable
	{
		...
		private bool IsSimpleTypeNonW8PFrameworkAPI()
		{
			...
			if (runtimeAssembly.IsFrameworkAssembly())
			{
				...
				if (global::System.Reflection.MetadataToken.IsNullToken(invocableAttributeCtorToken) || !CustomAttribute.IsAttributeDefined(this.GetRuntimeModule(), this.MetadataToken, invocableAttributeCtorToken))
				{
					return true;
				}
			}
			...
			if (types == null || types.Length == 0)
			{
				...
				if (types == null)
				{
					for (int i = 1; i < methodCandidates.Count; i++)
					{
						...
						if (!global::System.DefaultBinder.CompareMethodSigAndName(methodInfo2, methodInfo))
						{
